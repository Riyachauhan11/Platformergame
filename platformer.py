...
bg_img = pygame.image.load("bg0.png").convert_alpha()
...
playimg = pygame.image.load("start_btn.png").convert_alpha()
play_img = pygame.transform.scale(playimg, (tile_size*6, tile_size*3))
exitimg = pygame.image.load("exit_btn.png").convert_alpha()
...
game_won = pygame.mixer.Sound('tada.wav')
...
def reset_level(level):
    ...
    if path.exists(f"D:\Downloads\level{level}_data"):
        pickle_in = open(f'D:\Downloads\level{level}_data', 'rb')
...
class Player():
    ...
    def reset(self, x, y):
        ...
        for num in range(1, 12):
            img_right = pygame.image.load(f"p1_walk{num}.png").convert_alpha()
...
class Platform(pygame.sprite.Sprite):
    def __init__(self, image, x, y, move_x, move_y):
        pygame.sprite.Sprite.__init__(self)
        img = pygame.image.load(f'{image}.png')
